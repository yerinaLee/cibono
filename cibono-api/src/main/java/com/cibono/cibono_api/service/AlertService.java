...
public class AlertService {
    ...
    public List<AlertEventDto> listEvents(long userId, Boolean isRead) {
        ...
        for (AlertEvent ev : events) {
            ...
            Integer saving = (deal.getOriginalPrice() != null)
                    ? deal.getOriginalPrice() - deal.getDealPrice() : null;

            AlertEventDto.DealSummary dealSummary = new AlertEventDto.DealSummary(
                    deal.getId(), deal.getItemName(), deal.getDealPrice(),
                    deal.getOriginalPrice(), saving, deal.getEndsAt(), deal.getStoreId());
            ...
            result.add(new AlertEventDto(
                    ev.getId(), ev.isSeen(), ev.getTriggeredAt(), ev.getReadAt(),
                    dealSummary, ruleSummary));
        }
        return result;
    }
