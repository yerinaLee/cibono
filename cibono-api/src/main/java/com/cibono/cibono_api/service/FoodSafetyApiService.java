...
public class FoodSafetyApiService {
	...
	public List<RecipeDto.RecipeCard> searchByIngredient(String ingredient) {
		try {
			...
			if (json == null || json.stripLeading().startsWith("<")) {
				log.warn("[FoodSafety] searchByIngredient({}) → HTML/XML 응답 (API 한도초과 또는 오류): {}", ingredient,
						json == null ? "null" : json.substring(0, Math.min(300, json.length())));
				return List.of();
			}
	...
	private RecipeDto.RecipeDetail parseDetail(JsonNode row) {
		...
		String desc = (!category.isBlank() && !way.isBlank()) ? "[" + category + "] " + way
