...
public class InventoryController {
	...
	public List<InventoryResponse> list() {
		...
		Map<Integer, String> catMap = foodCategoryRepository.findAll().stream()
				.collect(Collectors.toMap(FoodCategory::getId, FoodCategory::getName));
		
		return items.stream()
				.map(inv -> new InventoryResponse(inv.getId(), inv.getUserId(), inv.getItemName(), inv.getQuantity(),
						inv.getUnit(), inv.getStorage(),
						inv.getPurchasedAt() != null ? inv.getPurchasedAt().toString() : null,
						inv.getExpiresAt() != null ? inv.getExpiresAt().toString() : null, inv.getCategoryId(),
	...
	public void delete(@PathVariable long id) {
		Inventory inv = inventoryRepository.findById(id)
				.orElseThrow(() -> new IllegalArgumentException("inventory not found"));
		if (!inv.getUserId().equals(UserContext.userId()))
	...
	public List<Inventory> bulkAdd(@RequestBody List<Inventory> reqs) {
		...
		return reqs.stream().map(req -> {
			...
			return inventoryRepository.findFirstByUserIdAndItemNameIgnoreCase(userId, normalizedName).map(existing -> {
