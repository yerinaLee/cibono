...
public class AlertRuleController {
    ...
    public Map<String, List<RuleDto>> listRules(
        ...
        List<PriceAlert> rules = (isEnabled != null)
                ? priceAlertRepository.findByUserIdAndIsEnabled(userId, isEnabled)
    ...
    public RuleDto updateRule(@PathVariable Long id, @RequestBody RuleRequest req) {
        ...
        if (req.thresholdPrice() != null) rule.setAnchorPrice(req.thresholdPrice());
        if (req.isEnabled() != null) rule.setEnabled(req.isEnabled());
    ...
    private RuleDto toDto(PriceAlert r) {
        return new RuleDto(r.getId(), new RuleDto.ItemInfo(r.getItemName()),
                r.getAnchorPrice(), "below", r.isEnabled(), r.getCreatedAt());
    }

    record RuleRequest(String itemName, Integer thresholdPrice, String condition, Boolean isEnabled) {}

    record RuleDto(
            Long id, ItemInfo item, Integer thresholdPrice,
            String condition, boolean isEnabled, OffsetDateTime createdAt
    ) {
