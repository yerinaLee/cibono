...
public class DealController {
    ...
    private DealDto toDto(Deal d) {
        Double discountRate = (d.getOriginalPrice() != null && d.getOriginalPrice() > 0)
                ? Math.round((1.0 - (double) d.getDealPrice() / d.getOriginalPrice()) * 1000.0) / 10.0
                : null;
        return new DealDto(d.getId(), new DealDto.ItemInfo(d.getItemName()),
                new DealDto.StoreInfo(d.getStoreId()),
                d.getDealPrice(), d.getOriginalPrice(), discountRate,
    ...
    private DealDetailDto toDetailDto(Deal d) {
        ...
        Double discountRate = (d.getOriginalPrice() != null && d.getOriginalPrice() > 0)
                ? Math.round((1.0 - (double) d.getDealPrice() / d.getOriginalPrice()) * 1000.0) / 10.0
                : null;
        return new DealDetailDto(d.getId(), new DealDetailDto.ItemInfo(d.getItemName()),
                new DealDetailDto.StoreInfo(d.getStoreId()),
                d.getDealPrice(), d.getOriginalPrice(), discountRate, saving,
                d.getStartsAt(), d.getEndsAt(), isActive);
    }

    record DealDto(
            Long id, ItemInfo item, StoreInfo store,
            Integer dealPrice, Integer originalPrice, Double discountRate,
    ...
    record DealDetailDto(
            Long id, ItemInfo item, StoreInfo store,
            Integer dealPrice, Integer originalPrice, Double discountRate, Integer saving,
